...
class AuthController extends Controller
{
    ...
    public function checkRegisterEmail($email_token)
    {
        ...
        if (!User::where('email_verify_token', $email_token)->exists()) {
            redirect('https://rss-syand.netlify.app/thanks/invalid');
        } else {
            $user = User::where('email_verify_token', $email_token)->first();
            // 本登録済みユーザーか
            if ($user->email_verified_at) {
                // logger("status". $user->email_verified );
                return redirect('https://rss-syand.netlify.app/thanks/exist');
            }
            // ユーザーステータス更新
            $user->email_verified_at = Carbon::now();
            if ($user->save()) {
                return redirect('https://rss-syand.netlify.app/thanks/success');
            } else {
                return redirect('https://rss-syand.netlify.app/thanks/fail');
            }
        }
    }
